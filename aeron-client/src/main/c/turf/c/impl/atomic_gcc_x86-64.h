...
#define turf_signalFenceAcquire() do { __asm__ volatile("" ::: "memory"); } while (0)
#define turf_signalFenceRelease() do { __asm__ volatile("" ::: "memory"); } while (0)
#define turf_signalFenceSeqCst() do { __asm__ volatile("" ::: "memory"); } while (0)

#define turf_threadFenceConsume() (0)
#define turf_threadFenceAcquire() do { __asm__ volatile("" ::: "memory"); } while(0)
#define turf_threadFenceRelease() do { __asm__ volatile("" ::: "memory"); } while(0)
#if TURF_CPU_X64
#define turf_threadFenceSeqCst() do { __asm__ volatile("lock; orl $0, (%%rsp)" ::: "memory"); } while (0)
#else
#define turf_threadFenceSeqCst() do { __asm__ volatile("lock; orl $0, (%%esp)" ::: "memory"); } while (0)
#endif
...
TURF_C_INLINE uint8_t turf_compareExchange8Relaxed(turf_atomic8_t* object, uint8_t expected, uint8_t desired) {
    uint8_t previous;
    __asm__ volatile("lock; cmpxchgb %2, %1" : "=a"(previous), "+m"(object->nonatomic) : "q"(desired), "0"(expected));
    return previous;
}
...
TURF_C_INLINE intreg_t turf_compareExchangeWeak8Relaxed(turf_atomic8_t* object, uint8_t* expected, uint8_t desired) {
    ...
    uint8_t previous;
    intreg_t matched;
    __asm__ volatile("lock; cmpxchgb %2, %1" : "=a"(previous), "+m"(object->nonatomic) : "q"(desired), "0"(e));
    matched = (previous == e);
    if (!matched)
        *expected = previous;
    return matched;
...
TURF_C_INLINE uint8_t turf_exchange8Relaxed(turf_atomic8_t* object, uint8_t desired) {
    uint8_t previous;
    __asm__ volatile("xchgb %0, %1" : "=r"(previous), "+m"(object->nonatomic) : "0"(desired));
    return previous;
}

TURF_C_INLINE uint8_t turf_fetchAdd8Relaxed(turf_atomic8_t* object, int8_t operand) {
    uint8_t previous;
    __asm__ volatile("lock; xaddb %0, %1" : "=r"(previous), "+m"(object->nonatomic) : "0"(operand));
    return previous;
}
...
TURF_C_INLINE uint16_t turf_compareExchange16Relaxed(turf_atomic16_t* object, uint16_t expected, uint16_t desired) {
    uint16_t previous;
    __asm__ volatile("lock; cmpxchgw %2, %1" : "=a"(previous), "+m"(object->nonatomic) : "q"(desired), "0"(expected));
    return previous;
}
...
TURF_C_INLINE intreg_t turf_compareExchangeWeak16Relaxed(turf_atomic16_t* object, uint16_t* expected, uint16_t desired) {
    ...
    uint16_t previous;
    intreg_t matched;
    __asm__ volatile("lock; cmpxchgw %2, %1" : "=a"(previous), "+m"(object->nonatomic) : "q"(desired), "0"(e));
    matched = (previous == e);
    if (!matched)
        *expected = previous;
    return matched;
...
TURF_C_INLINE uint16_t turf_exchange16Relaxed(turf_atomic16_t* object, uint16_t desired) {
    uint16_t previous;
    __asm__ volatile("xchgw %0, %1" : "=r"(previous), "+m"(object->nonatomic) : "0"(desired));
    return previous;
}

TURF_C_INLINE uint16_t turf_fetchAdd16Relaxed(turf_atomic16_t* object, int16_t operand) {
    uint16_t previous;
    __asm__ volatile("lock; xaddw %0, %1" : "=r"(previous), "+m"(object->nonatomic) : "0"(operand));
    return previous;
}
...
TURF_C_INLINE uint32_t turf_compareExchange32Relaxed(turf_atomic32_t* object, uint32_t expected, uint32_t desired) {
    ...
    // "=a"(previous) means the __asm__ block outputs EAX to previous, because CMPXCHG
    ...
    uint32_t previous;
    __asm__ volatile("lock; cmpxchgl %2, %1" : "=a"(previous), "+m"(object->nonatomic) : "q"(desired), "0"(expected));
    return previous;
}
...
TURF_C_INLINE intreg_t turf_compareExchangeWeak32Relaxed(turf_atomic32_t* object, uint32_t* expected, uint32_t desired) {
    ...
    uint32_t previous;
    intreg_t matched;
    __asm__ volatile("lock; cmpxchgl %2, %1" : "=a"(previous), "+m"(object->nonatomic) : "q"(desired), "0"(e));
    matched = (previous == e);
    if (!matched)
        *expected = previous;
    return matched;
...
TURF_C_INLINE uint32_t turf_exchange32Relaxed(turf_atomic32_t* object, uint32_t desired) {
    ...
    uint32_t previous;
    __asm__ volatile("xchgl %0, %1" : "=r"(previous), "+m"(object->nonatomic) : "0"(desired));
    return previous;
}
...
TURF_C_INLINE uint32_t turf_fetchAdd32Relaxed(turf_atomic32_t* object, int32_t operand) {
    ...
    uint32_t previous;
    __asm__ volatile("lock; xaddl %0, %1" : "=r"(previous), "+m"(object->nonatomic) : "0"(operand));
    return previous;
}
...
TURF_C_INLINE uint64_t turf_compareExchange64Relaxed(turf_atomic64_t* object, uint64_t expected, uint64_t desired) {
    ...
    // opcodes.
    uint64_t previous;
    __asm__ volatile("lock; cmpxchgq %2, %1" : "=a"(previous), "+m"(object->nonatomic) : "q"(desired), "0"(expected));
    return previous;
}
...
TURF_C_INLINE intreg_t turf_compareExchangeWeak64Relaxed(turf_atomic64_t* object, uint64_t* expected, uint64_t desired) {
    ...
    uint64_t previous;
    intreg_t matched;
    __asm__ volatile("lock; cmpxchgq %2, %1" : "=a"(previous), "+m"(object->nonatomic) : "q"(desired), "0"(e));
    matched = (previous == e);
    if (!matched)
        *expected = previous;
    return matched;
...
TURF_C_INLINE uint64_t turf_exchange64Relaxed(turf_atomic64_t* object, uint64_t desired) {
    uint64_t previous;
    __asm__ volatile("xchgq %0, %1" : "=r"(previous), "+m"(object->nonatomic) : "0"(desired));
    return previous;
}

TURF_C_INLINE uint64_t turf_fetchAdd64Relaxed(turf_atomic64_t* object, int64_t operand) {
    uint64_t previous;
    __asm__ volatile("lock; xaddq %0, %1" : "=r"(previous), "+m"(object->nonatomic) : "0"(operand));
    return previous;
}
