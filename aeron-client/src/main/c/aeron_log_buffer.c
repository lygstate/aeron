    ...
    aeron_log_buffer_t **log_buffer, aeron_image_os_ipc_t *os_ipc, int64_t correlation_id, bool pre_touch)
{
    ...
    *log_buffer = NULL;
    ...
    if (aeron_map_existing_log(&_log_buffer->mapped_raw_log, os_ipc, pre_touch) < 0)
    {
        aeron_set_err(aeron_errcode(), "could not map existing file buffer_id: %" PRId64 ": %s", os_ipc->buffer_id, aeron_errmsg());
