    ...
    aeron_system_counters_t *system_counters)
{
    ...
    *publication = NULL;
    ...
    _pub->log_file_name_length = aeron_log_buffer_filename_create(context->anonymous_file_mapping ? NULL : context->aeron_dir,
        _pub->log_file_name, sizeof(_pub->log_file_name) - 1, AERON_LOG_BUFFER_TYPE_NETWORK,
    ...
    aeron_network_publication_t *publication, int64_t now_ns, int32_t active_term_id, int32_t term_offset)
{
    ...
    if (now_ns > (publication->time_of_last_setup_ns + AERON_NETWORK_PUBLICATION_SETUP_TIMEOUT_NS))
    {
        ...
        if ((result = aeron_send_channel_sendmmsg(publication->endpoint, msgvec, 1)) != (int)iov[0].iov_len)
        {
            if (result >= 0)
            {
    ...
    bool signal_end_of_stream)
{
    ...
    if (now_ns > (publication->time_of_last_send_or_heartbeat_ns + AERON_NETWORK_PUBLICATION_HEARTBEAT_TIMEOUT_NS))
    {
        ...
        if ((bytes_sent = aeron_send_channel_sendmmsg(publication->endpoint, msgvec, 1)) != (int)iov[0].iov_len)
        {
            if (bytes_sent >= 0)
            {
                aeron_counter_increment(publication->short_sends_counter, 1);
...
int aeron_network_publication_resend(void *clientd, int32_t term_id, int32_t term_offset, size_t length)
{
    ...
    if (bottom_resend_window <= resend_position && resend_position < sender_position)
    {
        ...
        do
        {
            ...
            int sendmmsg_result;
            if ((sendmmsg_result = aeron_send_channel_sendmmsg(publication->endpoint, msgvec, 1)) != (int)iov[0].iov_len)
            {
