    ...
    aeron_system_counters_t *system_counters)
{
    ...
    *image = NULL;
    ...
    int path_length = aeron_publication_image_location(_image->log_file_name, sizeof(_image->log_file_name), context->aeron_dir, correlation_id);
    ...
    {
        aeron_free(_image);
        aeron_set_err(aeron_errcode(), "error mapping network raw log %s: %s", _image->log_file_name, aeron_errmsg());
        return -1;
    }
